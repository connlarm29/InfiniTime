...

{
  ...
  // Set the background
  lv_obj_set_style_local_bg_color(lv_scr_act(), LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0xFF00F7));
  ...
  if (obj == backgroundAction) {
    if (event == LV_EVENT_CLICKED) {
      ...
      if (isOn) {
        ...
        lv_obj_set_style_local_bg_color(lv_scr_act(), LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x000000));
        lv_obj_set_style_local_text_color(flashLight, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0xFF00F7));
      }
    }
  }
}
